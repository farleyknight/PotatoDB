...
HashJoinExec::HashJoinExec(MRef<ExecCtx> exec_ctx,
...
hash_t HashJoinExec::compute_hash(Ref<Tuple> tuple,
                                  Ref<Schema> schema,
                                  Vec<BaseExpr> exprs)
{
  hash_t curr_hash = 0;
  ...
  for (const auto &expr : exprs) {
    ...
    Value val = expr.eval(tuple, schema);
    // If this produces a value,
    if (!val.is_null()) {
      // We combine the hash of that value into our current hash.
      curr_hash = HashUtil::combine_hashes(curr_hash, HashUtil::hash_value(val));
    }
  }
  return curr_hash;
}
...
void HashJoinExec::init() {
  ...
  while (left_->has_next()) {
    ...
    hash_t hash_value = compute_hash(left_tuple, left_schema(), plan_->left_keys());
  ...
  while (right_->has_next()) {
    ...
    hash_t hash_value = compute_hash(right_tuple, right_schema(), plan_->right_keys());
    ...
    for (auto &left_tuple : left_tuples) {
      if (match_found(left_tuple, right_tuple)) {
        MutVec<Value> values(col_total);
        for (size_t col_index = 0; col_index < col_total; ++col_index) {
          values[col_index] = make_value_at(col_index, left_tuple, right_tuple);
        }
...
Value HashJoinExec::make_value_at(int k, Ref<Tuple> left, Ref<Tuple> right) {
  return schema().column(k).expr().eval_join(left,  left_schema(),
                                             right, right_schema());
}

bool HashJoinExec::match_found(Ref<Tuple> left, Ref<Tuple> right) {
  auto result = plan_->pred().eval_join(left, left_schema(),
                                        right, right_schema());
  return result.get_as<bool>();
}
...
Ref<Schema> HashJoinExec::left_schema()  { return left_->schema(); }
Ref<Schema> HashJoinExec::right_schema() { return right_->schema(); }
