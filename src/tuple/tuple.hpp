...
public:
  ...
  void
  copy_from(const Tuple& tuple) {
    buffer_.copy_from(tuple.buffer_);
  }

  bool
  eq(const Tuple& tuple) {
    if (size() != tuple.size()) {
      return false;
    }
