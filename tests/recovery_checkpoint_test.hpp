...
TEST(RecoveryCheckpointTest, DISABLED_CheckpointTest) {
  ...
  db.set_log_timeout(std::chrono::seconds(15));
  ...
  auto pages = db.buff_mgr().all_pages();
  ...
  for (size_t i = 0; i < pool_size; i++) {
    ...
    if (page_id.is_valid()) {
      db.file_mgr().read_page(page_id, buffer);

      if (std::memcmp(buffer.data(), page->data()) != 0) {
  ...
  auto persistent_lsn = db.log_mgr().persistent_lsn();
  ...
  for (size_t i = 0; i < pool_size; ++i) {
    ...
    if (page_id.is_valid() && page->lsn() > persistent_lsn) {
      all_pages_lte = false;
      break;
    }
  }
